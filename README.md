...
```bash
npm install @_djhhh/ddo-cli
```

安装后需要使用 `npx` 运行：
```bash
npx @_djhhh/ddo-cli init
npx @_djhhh/ddo-cli start
```
...
> 本项目LLM使用OpenRouter中转站

以下模型配置信息不会写入 `~/.ddo`，需要手动配置：

```bash
set DDO_LLM_MODEL=minimax/minimax-m2.5 （推荐配置）
set DDO_LLM_RAG_MODEL=qwen/qwen3-embedding-8b（推荐配置）
...
**本地安装后**：
```bash
npx @_djhhh/ddo-cli init --data-dir D:\custom\ddo-data
```
...
**本地安装后**：
```bash
npx @_djhhh/ddo-cli start --skip-repl
```
...
**本地安装后**：
```bash
npx @_djhhh/ddo-cli status
npx @_djhhh/ddo-cli stop
```
...
会。`llm-py` 的 Python 依赖已在打包时自动安装到 vendor 目录，无需手动安装。
