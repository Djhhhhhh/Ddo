...
exports.mcpHelpCommand = exports.mcpDisconnectCommand = exports.mcpConnectCommand = exports.mcpRemoveCommand = exports.mcpCallCommand = exports.mcpToolsCommand = exports.mcpTestCommand = exports.mcpAddCommand = exports.mcpListCommand = void 0;
...
function validateTransportType(value) {
...
function isRequiredProperty(tool, key) {
...
function coerceArrayItemValue(value, itemSchema) {
...
function parseToolArgumentValue(prop, value) {
...
function validateToolArgumentInput(prop) {
    return (value) => {
        try {
...
function buildToolPromptDefs(tool) {
...
function printToolSchema(schema) {
    if (!schema) {
        console.log(chalk_1.default.gray('    参数: 未提供 inputSchema'));
        return;
    }
    console.log(chalk_1.default.gray('    inputSchema:'));
...
exports.mcpAddCommand = {
    ...
    handler: async ({ args, nlpParameters, rl }) => {
        ...
        if (allCollected) {
            ...
            try {
                ...
                if (normalizedType === 'stdio' && collected.name) {
                    console.log(chalk_1.default.gray('stdio 模式请直接填写可执行命令，不要使用 cmd /c'));
                    const stdioInitial = {};
                    if (legacyConfig) {
                    ...
                    if (nlpParameters?.command)
                        stdioInitial.command = String(nlpParameters.command);
                    ...
                    if (nlpParameters?.url)
                        httpInitial.url = String(nlpParameters.url);
                    if (nlpParameters?.headers)
                        httpInitial.headers = stringifyValue(nlpParameters.headers);
...
exports.mcpTestCommand = {
    ...
    handler: async ({ args, rl }) => {
        ...
        const apiClient = (0, api_client_1.getApiClient)();
        try {
            console.log(chalk_1.default.gray('正在测试 MCP 连接与初始化...'));
            ...
            console.log(chalk_1.default.gray(`状态: ${result.status || 'unknown'}`));
            console.log(chalk_1.default.gray(`Transport 可达: ${reachable ? 'yes' : 'no'}`));
            console.log(chalk_1.default.gray(`Initialize 成功: ${initializeSucceeded ? 'yes' : 'no'}`));
            console.log(chalk_1.default.gray(`协议就绪: ${protocolReady ? 'yes' : 'no'}`));
            if (typeof result.latencyMs === 'number') {
                console.log(chalk_1.default.gray(`耗时: ${result.latencyMs}ms`));
            }
            if (result.serverProtocolVersion) {
                console.log(chalk_1.default.gray(`协议版本: ${result.serverProtocolVersion}`));
            }
            if (result.serverInfo) {
                console.log(chalk_1.default.gray(`服务信息: ${stringifyValue(result.serverInfo)}`));
            }
            ...
            if (result.error) {
                console.log(chalk_1.default.yellow(`错误信息: ${result.error}`));
            }
            console.log();
        }
        catch (err) {
...
exports.mcpToolsCommand = {
    ...
    handler: async ({ args, rl }) => {
        ...
        const apiClient = (0, api_client_1.getApiClient)();
        try {
            console.log(chalk_1.default.gray('正在获取 MCP 工具列表...'));
            const result = await apiClient.getMcpTools(uuid);
            const tools = result.tools || [];
            console.log(chalk_1.default.cyan('\nMCP 工具列表:'));
            console.log(chalk_1.default.gray('─'.repeat(40)));
            if (tools.length === 0) {
                console.log(chalk_1.default.gray('  当前 MCP 未返回任何工具'));
            }
            else {
                for (const tool of tools) {
                    console.log(`  ${chalk_1.default.yellow(tool.name)}${tool.title ? chalk_1.default.gray(` (${tool.title})`) : ''}`);
                    if (tool.description) {
                        console.log(chalk_1.default.gray(`    描述: ${tool.description}`));
                    }
            ...
            console.log(chalk_1.default.red('获取 MCP 工具列表失败:'), err instanceof Error ? err.message : String(err));
        }
        return { shouldContinue: true, outputType: index_1.CommandType.Command };
    },
};
exports.mcpCallCommand = {
    name: 'mcp-call',
    ...
    handler: async ({ args, rl }) => {
        ...
        try {
            ...
            console.log(chalk_1.default.yellow('获取工具列表失败，将继续按手动输入处理'));
        }
        if (tools.length > 0) {
            console.log(chalk_1.default.gray('可用工具:'));
            for (const tool of tools) {
                console.log(chalk_1.default.gray(`  - ${tool.name}${tool.title ? ` (${tool.title})` : ''}`));
            }
        }
        ...
        if (rawArgs) {
            ...
            if (matchedTool.description) {
                console.log(chalk_1.default.gray(matchedTool.description));
            }
        ...
        try {
            console.log(chalk_1.default.gray(`正在调用工具 ${toolName}...`));
            const result = await apiClient.callMcpTool(uuid, toolName, parsedArgs);
            console.log(chalk_1.default.cyan('\n工具调用结果:'));
            console.log(chalk_1.default.gray('─'.repeat(40)));
            console.log(chalk_1.default.gray(`工具: ${toolName}`));
            console.log(chalk_1.default.gray(`是否错误: ${result.isError ? 'yes' : 'no'}`));
            if (result.content !== undefined) {
                console.log(chalk_1.default.gray('content:'));
                console.log(stringifyValue(result.content));
            }
            if (result.structuredContent !== undefined) {
                console.log(chalk_1.default.gray('structuredContent:'));
                console.log(stringifyValue(result.structuredContent));
            }
            if (result.raw !== undefined) {
                console.log(chalk_1.default.gray('raw:'));
            ...
            console.log(chalk_1.default.red('调用 MCP 工具失败:'), err instanceof Error ? err.message : String(err));
        }
        return { shouldContinue: true, outputType: index_1.CommandType.Command };
    },
};
...
exports.mcpConnectCommand = {
    ...
    handler: async ({ args, rl }) => {
        ...
        const apiClient = (0, api_client_1.getApiClient)();
        try {
            console.log(chalk_1.default.gray('正在建立连接...'));
            ...
            console.log(chalk_1.default.red('建立连接失败:'), err instanceof Error ? err.message : String(err));
        }
        return { shouldContinue: true, outputType: index_1.CommandType.Command };
    },
};
exports.mcpDisconnectCommand = {
    name: 'mcp-disconnect',
    ...
    handler: async ({ args, rl }) => {
        ...
        const apiClient = (0, api_client_1.getApiClient)();
        try {
            console.log(chalk_1.default.gray('正在断开连接...'));
            ...
            console.log(chalk_1.default.red('断开连接失败:'), err instanceof Error ? err.message : String(err));
        }
        return { shouldContinue: true, outputType: index_1.CommandType.Command };
    },
};
exports.mcpHelpCommand = {
    ...
    handler: async () => {
        ...
        console.log(`  ${chalk_1.default.yellow('mcp-add')}          - 添加 MCP 配置（支持渐进式交互）`);
        console.log(`  ${chalk_1.default.yellow('mcp-test')}         - 测试 MCP 连接与初始化`);
        console.log(`  ${chalk_1.default.yellow('mcp-connect')}      - 建立并保持 MCP 连接`);
        console.log(`  ${chalk_1.default.yellow('mcp-disconnect')}   - 断开 MCP 连接`);
        console.log(`  ${chalk_1.default.yellow('mcp-tools')}        - 查看 MCP 工具列表与参数 Schema`);
        console.log(`  ${chalk_1.default.yellow('mcp-call')}         - 测试调用指定 MCP 工具`);
        ...
        console.log(`  ${chalk_1.default.gray('/mcp-add "my-mcp" streamable_http "http://localhost:3000/mcp"')}`);
        console.log(`  ${chalk_1.default.gray('/mcp-connect 12345678-1234-1234-1234-123456789abc')}`);
        console.log(`  ${chalk_1.default.gray('/mcp-disconnect 12345678-1234-1234-1234-123456789abc')}`);
        console.log(`  ${chalk_1.default.gray('/mcp-tools 12345678-1234-1234-1234-123456789abc')}`);
        console.log(`  ${chalk_1.default.gray('/mcp-call 12345678-1234-1234-1234-123456789abc get_weather "{\"city\":\"Shanghai\"}"')}`);
        console.log();
        return { shouldContinue: true, outputType: index_1.CommandType.Command };
    },
};
