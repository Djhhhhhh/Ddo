...
- MCP 连接管理：新增 `/mcp-connect` 和 `/mcp-disconnect` 命令，调用 server-go 新增的持久连接 API（发现日期：2026-04-25）
- API 客户端扩展：新增 API 方法时需同步在 `api-client.ts` 添加方法、在返回对象中暴露、在命令中调用（发现日期：2026-04-25）
