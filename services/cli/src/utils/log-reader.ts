...
/** 获取日志文件路径 */
export function getLogFilePath(logsDir: string, service: string): string {
  return `${logsDir}/${service}.log`;
}

/** 解析 since 参数，返回毫秒时间戳 */
export function parseSinceTime(since: string): number {
  const now = Date.now();
...
/** 尝试从日志行解析时间戳和级别 */
export function parseLogLine(line: string): { timestamp?: Date; level?: string; message: string } {
  // 尝试匹配常见日志格式：
...
/** 检查日志级别是否匹配 */
export function matchLogLevel(lineLevel: string | undefined, filterLevel: string): boolean {
...
/** 高亮日志级别 */
export function highlightLogLevel(line: string): string {
  // 使用 ANSI 颜色高亮不同级别
  // ERROR - 红色
  // WARN - 黄色
  // INFO - 蓝色/青色
...
/** 获取 MySQL 容器日志 */
export async function getMySQLLogs(
...
/** 读取多个服务的日志 */
export async function readMultipleLogs(
  logsDir: string,
  services: string[],
  lines: number,
  since?: string,
  ...
  for (const service of services) {
    if (service === 'mysql') continue; // MySQL 需要特殊处理
