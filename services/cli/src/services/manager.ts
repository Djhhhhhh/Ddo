...
export function createServiceManager(config: ManagerConfig) {
  ...
  async function startService(service: ServiceDefinition): Promise<StartResult> {
    ...
    try {
      ...
      if (startupStrategy === 'process') {
        ...
        logger.success(`${service.displayName} 已启动 (PID: ${child.pid})`);
        return { success: true, pid: child.pid };
      }

      // 等待服务就绪
      const healthy = await waitForHealthy(service.healthUrl, service.startupTimeoutMs ?? 30000, 1000);
  ...
  async function stopService(serviceName: string, displayName: string): Promise<StopResult> {
    ...
    killProcess(pid, process.platform === 'win32');
    await sleep(1000);
    ...
    // 再次确认进程是否已停止
    if (isProcessRunning(pid)) {
      logger.warn(`${displayName} 未响应，再次强制终止...`);
